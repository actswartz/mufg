...
Automation is a superpower. In a large company, you don't give superpowers to everyone. You acting as **S<student_id>-user** (an Org Admin) will learn how to create a "Junior Engineer" user who can run automation but cannot change it.
...
2.  **Name:** `Junior Network Admins`
3.  **Organization:** Select your `Org-S<student_id>`.
4.  Click **Save**.

**✅ Success Criteria:** You have created a new functional group (Team) within your Organization.
...
2.  **Username:** `S<student_id>-junior` (e.g., `S1-junior`).
3.  **Organization:** Select your `Org-S<student_id>`.
4.  **Password:** `800-ePlus`
5.  Click **Save**.
6.  Go to **Teams** -> **Junior Network Admins** -> **Users** tab.
7.  Click **Add** and select your new `S<student_id>-junior` user.

**✅ Success Criteria:** You have created a new user and assigned them to the "Junior Network Admins" team.
...
**✅ Success Criteria:** The Junior Team now has the authority to run (but not edit) the Gather Facts template.
...
### Step-by-Step:
1.  Log out of AWX.
2.  Log back in as the junior user: **`S<student_id>-junior`**.
...
**✅ Success Criteria:** You have successfully demonstrated "Least Privilege" access.
...
In this lab, you gave the `S<student_id>-junior` user the **'Execute'** role on a specific template. Notice that they did *not* get the ability to see the Credentials. This is a brilliant piece of security engineering: AWX allows a user to *use* a password without ever *knowing* the password. The junior user can log into a router using the `Cisco Router Login` account, but they can't see the `800-ePlus` password in the UI, and they can't even see the stars (********) because they don't have access to the Credential object itself.
